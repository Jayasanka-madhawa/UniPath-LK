...
Examples:
{"action":"compare_courses_tool","args":{"course_code_1":"013","course_code_2":"012"}}
{"action":"lookup_section_tool","args":{"section_id":"1.7"}}
{"action":"search_handbook_tool","args":{"query":"SLIATE registration rule"}}
{"action":"final_answer","args":{"answer":"..."}}

Rules:
- Use structured tools for course comparison and course facts.
- Use RAG tools for policy/process questions.
- Never invent Z-scores or cutoffs.
- Label facts clearly in the final answer.
- Include handbook citations when using RAG.

For compare questions, use find_course_tool first OR pass full course names.
Codes must be 3-digit like "013" and "012", never abbreviations like PSC.
"""
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Quote the relevant handbook rule. Include citation "
...
def run_agent(question: str, max_steps: int = 4) -> AgentResult:
    ...
    for _ in range(max_steps):
        ...
        if action == "final_answer":
            ...
            return AgentResult(question=question, answer=answer, tool_calls=tool_calls)

        observation = _execute(action, args)
